              ...
              {gb.basket.map((item) => (
                <div key={item.dish_id} className="bg-surface px-3 py-2 rounded-xl shadow-sm border border-hair-soft flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => gb.modify(item.dish_id, -1)} className="w-8 h-8 rounded-lg bg-inset flex items-center justify-center text-ink text-base font-bold active:scale-95 transition">-</button>
                    <span className="font-bold text-ink w-5 text-center text-base">{item.quantity}</span>
                    <button onClick={() => gb.modify(item.dish_id, 1)} className="w-8 h-8 rounded-lg bg-black text-white flex items-center justify-center text-base font-bold active:scale-95 transition">+</button>
