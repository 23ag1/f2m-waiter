...
def build_dish_constraints(derived_dir: Path | str) -> list[dict[str, Any]]:
    ...
    if not dish_cache_path.exists():
        return []
    dishes = json.loads(dish_cache_path.read_text(encoding="utf-8"))
    constraints: list[dict[str, Any]] = []
    for dish in dishes:
        constraints.append(_build_single_dish_constraints(dish))
...
def _build_single_dish_constraints(dish: dict[str, Any]) -> dict[str, Any]:
    dish_id = str(dish.get("dish_id", ""))
    ...
    contains_gluten = _flag_any(ingredients, hard_flags, "gluten", {"wheat", "noodles", "pasta"}, name_norm, ("паста", "лапша", "тесто", "gluten"))
    contains_lactose = _flag_any(ingredients, hard_flags, "lactose", {"milk", "cream", "cheese", "butter"}, name_norm, ("сыр", "сливоч", "молоч"))
    ...
    contains_fish_seafood = _flag_any(
        ...
        hard_flags,
        "fish_seafood",
        {"salmon", "tuna", "shrimp", "fish", "seafood", "fish_sauce", "oyster_sauce", "mussels", "squid"},
        name_norm,
        ("рыб", "морепроду", "кревет", "лосос", "тунц"),
    )
    ...
    contains_pork = _flag_any(ingredients, hard_flags, "pork", {"pork", "bacon", "ham"}, name_norm, ("свинин", "бекон"))
    ...
    contains_spicy = _flag_contains(ingredients, {"chili", "curry", "pepper"}) or ("остр" in name_norm)
    ...
    vegetarian_conflict = any(
        [
            ...
            contains_pork,
            contains_offal,
            _flag_contains(ingredients, {"beef", "lamb", "chicken", "turkey", "bacon", "ham"}),
            any(token in ingredients for token in ("beef", "lamb", "chicken", "turkey")),
            any(token in name_norm for token in ("говя", "баран", "куриц", "индейк", "шашлык")),
        ]
    )
...
def _evaluate_dish_block_reason(
    ...
    if "vegetarian" in dietary_rules:
        if bool(dietary.get("vegetarian_conflict", False)):
            return {"key": "hard_vegetarian_conflict", "type": "explicit_conflict", "source_evidence": "dietary_conflicts"}
        if "vegetarian_conflict" in unresolved or review_required:
            return {"key": "hard_vegetarian_unknown", "type": "unresolved", "source_evidence": "review_required_or_unresolved"}

    if "vegan" in dietary_rules:
        if bool(dietary.get("vegan_conflict", False)):
            return {"key": "hard_vegan_conflict", "type": "explicit_conflict", "source_evidence": "dietary_conflicts"}
        if "vegan_conflict" in unresolved or review_required:
            return {"key": "hard_vegan_unknown", "type": "unresolved", "source_evidence": "review_required_or_unresolved"}

    if "no_pork" in dietary_rules:
        if bool(dietary.get("no_pork_conflict", False)):
            return {"key": "hard_no_pork_conflict", "type": "explicit_conflict", "source_evidence": "dietary_conflicts"}
        if "no_pork_conflict" in unresolved or review_required:
            return {"key": "hard_no_pork_unknown", "type": "unresolved", "source_evidence": "review_required_or_unresolved"}
