...
from f2m_engine.domain.recommendation_settings import (
    context_tags_from_settings,
    load_recommendation_settings,
)
from f2m_engine.pipelines.recommendation_engine import rank_dishes_for_cvp
...
STRICT_ALLERGY_CONSTRAINTS = {"peanut", "nuts", "nuts_peanut", "lactose", "egg", "soy", "fish_seafood"}
...
def score_dishes_for_user(
    ...
    ranked = rank_dishes_for_cvp(
        cvp_profile=cvp_profile,
        dish_cache=dish_cache,
        dish_ingredient_evidence=dish_ingredient_evidence,
    ...
    survivors = ranked.candidates
    shown_candidates = survivors[:top_k]
    user_profile_snapshot_id = hashlib.sha256(
        json.dumps(cvp_profile, sort_keys=True, ensure_ascii=False).encode("utf-8")
    ).hexdigest()[:16]
    request_id = log_recommendation_request(
        derived_dir=base,
        user_id=user_id,
        context_snapshot={
            "request_context_tags": request_tags,
            "raw_request_context": request_context or {},
            "recommendation_status": ranked.status,
        ...
        profile_version=cvp_profile.get("meta", {}).get("cvp_schema", "cvp_v1"),
        shown_candidates=shown_candidates,
        hard_filter_status_by_dish=ranked.hard_filter_status_by_dish,
    )
    for row in survivors:
        row["recommendation_request_id"] = request_id
        row["recommendation_status"] = ranked.status
    _write_outputs(
        base=base,
        audit_dir=audit_dir,
        scored_rows=survivors,
        top_k=top_k,
        constraint_audit_rows=ranked.constraint_audit_rows,
    )
    _write_recommendation_state(base=base, result=ranked, request_id=request_id)
    return ScoringStats(
        user_id=user_id,
        request_id=request_id,
        survivors=len(survivors),
        excluded=len(ranked.constraint_audit_rows) - len(survivors),
        top_k=top_k,
...
def _apply_hard_filters(
    ...
    for constraint in constraints:
        ...
        if normalized_constraint in STRICT_ALLERGY_CONSTRAINTS:
            # nuts_peanut покрывает и "nuts", и "peanut" одновременно
            if normalized_constraint == "nuts_peanut":
                if bool(hard_flags.get("nuts")) or bool(hard_flags.get("peanut")) \
                        or "nuts" in ingredients or "peanut" in ingredients:
                    excluded = True
                    reasons.append("hard_nuts_peanut")
            else:
...
def _write_recommendation_state(base: Path, result: Any, request_id: str) -> None:
    payload = {
        "request_id": request_id,
        "status": result.status,
        "requested_top_k": int(result.requested_top_k),
        "available_count": int(result.available_count),
        "partial_state": result.status == "partial",
        "empty_state": result.status == "empty",
        "backend_order_contract": "backend_order",
    }
    (base / "recommendation_state.json").write_text(
...
def _constraint_alias(key: str) -> str:
    mapping = {
        ...
        # Арахис и орехи объединяем в один constraint — пользователь может указать любой
        "peanut":         "nuts_peanut",
        "nuts":           "nuts_peanut",
        "nut":            "nuts_peanut",
        "орехи":          "nuts_peanut",
        "арахис":         "nuts_peanut",
        "nuts_peanut":    "nuts_peanut",
    }
