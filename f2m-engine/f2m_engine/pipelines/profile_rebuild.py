...
def show_profile(derived_dir: Path | str, user_id: int) -> str:
    ...
    cvp_profile = profile.get("cvp_profile") or build_cvp_profile(
        profile_cache_row=profile,
        constraint_rows=constraints.get(user_id, []),
    )
    lines = [
        f"user_id={user_id}",
        f"hard_constraints={hard}",
        f"cvp_version={cvp_profile['cvp_version']}",
        f"human_readable_description={cvp_profile['human_readable_description']}",
...
def _materialize_cache_rows(
    ...
    for user_id in users:
        ...
        row["cvp_profile"] = build_cvp_profile(
            profile_cache_row=row,
            constraint_rows=constraints_by_user.get(user_id, []),
        )
        rows.append(row)
