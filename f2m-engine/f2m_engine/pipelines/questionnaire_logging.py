...
def write_prefilter_candidates(
    ...
    for row in rows:
        _append_jsonl(
            Path(derived_dir),
            "questionnaire_candidates_prefilter.jsonl",
            {
                "request_id": request_id,
                "mode": mode,
...
def write_filter_decisions(
    ...
    for row in rows:
        _append_jsonl(
            Path(derived_dir),
            "questionnaire_candidate_filter_decisions.jsonl",
            {
                "request_id": request_id,
                "mode": mode,
                "venue_normalized": venue_normalized,
...
def write_scored_candidates(
    ...
    for row in rows:
        _append_jsonl(Path(derived_dir), "questionnaire_candidates_scored.jsonl", row)
...
def write_presented_candidates(
    ...
    scored_name_by_id: dict[tuple[str, str], str] = {}
    for row in top_rows:
        key = (str(row.get("request_id", "")), str(row.get("dish_id", "")))
        dish_name = str(row.get("dish_name", "") or "").strip()
        if dish_name:
            scored_name_by_id[key] = dish_name
    for row in top_rows:
        req_id = str(row.get("request_id", ""))
        dish_id = str(row.get("dish_id", ""))
        resolved_name = str(row.get("dish_name", "") or "").strip() or scored_name_by_id.get((req_id, dish_id), "")
        _append_jsonl(
            Path(derived_dir),
            "questionnaire_recommendations_presented.jsonl",
            {
                "request_id": req_id,
                ...
                "menu_section": str(row.get("menu_section", "food_recommendations")),
                "warning_tags": row.get("warning_tags", []),
                ...
                "soft_negative_hit_key": str(row.get("soft_negative_hit_key", "")),
                "soft_negative_hit_keys": row.get("soft_negative_hit_keys", []),
                "soft_negative_rank_group": int(row.get("soft_negative_rank_group", 0)),
                "diversity_bonus": float(row["diversity_bonus"]),
                "intent_coverage_bonus": float(row.get("intent_coverage_bonus", 0.0)),
...
def log_questionnaire_outcome(
    ...
    outcomes = _read_jsonl(Path(derived_dir) / "questionnaire_outcomes.jsonl")
    if not occurred_at:
    ...
    _append_jsonl(Path(derived_dir), "questionnaire_outcomes.jsonl", row)
    return row
        ...
        dataset_rows.append(
            {
                ...
                "menu_section": scored_row.get("menu_section", "food_recommendations"),
                "warning_tags": scored_row.get("warning_tags", []),
                "display_tags": scored_row.get("display_tags", []),
                ...
                "soft_negative_hit_key": scored_row.get("soft_negative_hit_key", ""),
                "soft_negative_hit_keys": scored_row.get("soft_negative_hit_keys", []),
                "soft_negative_rank_group": scored_row.get("soft_negative_rank_group", 0),
                "diversity_bonus": scored_row.get("diversity_bonus", 0.0),
                "intent_coverage_bonus": scored_row.get("intent_coverage_bonus", 0.0),
...
def _append_jsonl(base_dir: Path, file_name: str, row: dict[str, Any]) -> None:
    for path in _runtime_paths(base_dir=base_dir, file_name=file_name):
...
def _runtime_paths(base_dir: Path, file_name: str) -> list[Path]:
    base = base_dir.resolve()
    project_root = Path(__file__).resolve().parents[2]
    paths: list[Path] = [base / file_name]
    default_derived = (project_root / "data" / "derived").resolve()
    root_path = (project_root / file_name).resolve()
    if base == default_derived and root_path not in paths:
